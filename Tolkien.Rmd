---
title: "Tolkien's Legendarium - Lexical Dispersion Plot"
...
newtheme <- theme_minimal(base_family = "Candara")+
  theme(plot.title = element_markdown(size = 10.5, color = "grey30", margin = margin(b = -8)),
...
## Keyword Locating
keyword_locate <- function(text_df, keyword) {
  kword_loc <- sapply(text_df$Text, function(x) 
    { loc <- tolower(x) %>% str_split(" ") %>% unlist()%>%
              str_which(tolower(keyword))
    if(length(loc) == 0) {NA}
    else {loc}
    })
...
#save(Legendarium_list, file = "data/Legendarium.RData")
```

### Lexical Dispersion Plot

```{r tiding, fig.width= 10.5, fig.height = 5}
load("data/Legendarium.RData")

Legendarium_lex_dispersion <- function(keyword) {
  Keyword_Loc_list <- lapply(Legendarium_list, keyword_locate, 
                             keyword = keyword) %>%
  `names<-`(Legendarium_titles)
...
Legendarium_lex_dispersion("tiding")
```
